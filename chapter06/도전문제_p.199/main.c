/* 도전문제. Implement element get_entry(ListType* L, int index).
Find and return index-th data. */

// test every code in directory 6.1 ~ 6.6
#include <stdio.h>
#include <stdlib.h>
...
int main(void)
{
    ...
    for (int i = 0; i < 5; i++) {
        head = insert_first(head, i);   // assign returned head pointer into head
        print_list(head);
    }
    for (int i = 4; i >= 0; i--)
        printf("get_entry(head, %d) = %d\n", i, get_entry(head, i));
    for (int i = 0; i < 5; i++) {
